...
impl<T, E: Weighted, ID: Clone + Hash + Eq + fmt::Debug> Graph<T, E, ID> {
    ...
    pub fn closest(&self, from: Rc<Route<ID>>, to: &HashSet<ID>) -> Option<Rc<Route<ID>>> {
        ...
        loop {
            ...
            let exits = self.nodes.get(&c_route.pos).unwrap();
    ...
    pub fn greedy_salesman(&self, start: ID) -> Option<Rc<Route<ID>>> {
        let mut to_visit: HashSet<ID> = self.nodes.keys().map(|k| k.clone()).collect();
    ...
    pub fn iter_salesman(&self, start: ID) -> Option<Rc<Route<ID>>> {
        let mut bpath: Vec<ID> = self.nodes.keys().map(|k| k.clone()).collect();
...
fn main() -> Result<(), GraphErr> {
    let mut g = Graph::new();
    for x in vec!["AA","BB","CC","DD","EE","FF"] {
        g.add_node(x, ());
    }
    g.add_edge("ab", "AA", "BB", 4)?;
    g.add_edge("ac", "AA", "CC", 2)?;
    g.add_edge("bc", "BB", "CC", 5)?;
    g.add_edge("bd", "BB", "DD", 10)?;
    g.add_edge("ce", "CC", "EE", 3)?;
    g.add_edge("ed", "EE", "DD", 4)?;
    g.add_edge("df", "DD", "FF", 11)?;
    println!("Hello graph : {:?}", g);

    let bestpath = g.shortest_path("AA", "GG");
    if let Some(bestpath2) = bestpath {
        println!("AA - DD = {}", bestpath2);
    } else {
        println!("No route");
    }
    println!("greedy AA = {}", g.greedy_salesman("AA").unwrap());
    println!("greedy BB = {}", g.greedy_salesman("BB").unwrap());
    println!("greedy CC = {}", g.greedy_salesman("CC").unwrap());
/*
